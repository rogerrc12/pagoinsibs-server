...
const { gcStorage, deleteFileGcStorage } = require('../../helpers/googleStorage');

const getBankPayments = async (req, res, next) => {
  const { bank_id } = req.query;
  
  try {
    const payments = await BankPayment.findAll({ where: { bankId: bank_id }, include: [{model: Correlative}, {model: User}], order: [ ['registerID', 'ASC'] ] });
...
const processCiserFile = async (req, res, next) => {
  try {
    ...
    const fileStream = await gcStorage.bucket(process.env.BUCKETNAME).file(filename).createReadStream();
    ...
    fileArray.map(item => {
      const object = item.reduce((result, field, i) => {
        let key = fileKeys[i].normalize("NFKD").replace(/[\u0300-\u036F]/g, "").replace(/\s+/g, "");
        result[key] = field;
        return result;
      }, {});
    ...
    if (!payments && !debits) {
      const error = new Error('No hay ningun pago para ser procesado en este lote. Revisa el archivo o intenta con' +
        ' otro.');
      error.statusCode = 404;
      throw error;
    }
      ...
      if (data[i].Estado.toLowerCase() === 'pagado') {
        ...
        await db.transaction(async trx => {
          ...
          if (fee.debit.debitType === 'recurrente') {
            ...
            await Debit.update({ statusId: 1, startPaymentDate: paymentDate, attempts: 0 }, { where: { id: debit.id }, transaction: trx })

          } else {
            
            const feePaymentDate = await FeeControl.findOne({ where: { statusId: 1, debitId: fee.debit.id }, attributes: ['paymentDate'] });
        ...
        await db.transaction(async trx => {
          ...
          if (newAttempts === 0) {
            await FeeControl.update({ statusId: 4 }, { 
              where: { 
                debitId: fee.debitId, 
                [Op.not]: { statusId: 3 } 
              } 
