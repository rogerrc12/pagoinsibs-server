...
const getPaymentDetails = async (req, res, next) => {
  ...
  try {
    const payment = await AccPayments.findByPk(id, { include: [{model: User}, {model: Supplier}, {model: Status}] });
...
const processPaymentToBank = async payment => {
  ...
  try {
    ...
    let correlative = await Correlative.findOne({
      where: {processed: false}, include: [{ model: BankPayment, where: { bankId: bank.id }  }]
    });
...
const postPaymentToBank = async function(req, res, next) {
  ...
  try {
    ...
    const paymentDetails = await AccPayments.findByPk(id, { include: [{ model: User }, { model: Supplier }, { model: Status }] });
