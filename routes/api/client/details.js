...
router.post('/transfer', 
  [
    verify,
    [ check('email', 'Debes ingresar un correo electrónico válido.').isEmail() ]
  ],
  async (req, res) => {
    ...
    try {
      ...
      if (transfer.length > 0) {
        ...
        const options = { 
          email,
          subject: `Detalle de la operación no. ${transfer_id}`, template: 'transfer_detail',
          variables: 
            JSON.stringify({ 
              name: req.user.name, transfer_id, description: transfer[0].description, 
              ...
              bank_name: transfer[0].bank_name, acc_type: transfer[0].acc_type, 
        ...
        return res.status(404).json({ errors: [{ msg: 'Ha ocurrido un error. Al parecer esta transacción no se encuentra registrada, verifica tu conexión a internet o contacta a soporte si el error continua.' }] })
      }

    } catch (error) {
      console.error(error.message);
      res.status(500).json({ errors: [{ msg: 'Ha ocurrido un error, pero es todo lo que sabemos. Por favor, verifica tu conexión a internet o contacta a soporte si el error continua.' }] })
    }
  }
)
