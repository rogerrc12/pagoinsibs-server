...
router.get('/:id', verify, async (req, res) => {
  ...
  try {
    const transfer = await db('users_transfers as ut').select([
    'ut.*', 'u.cedula', 'u.pay_id', 'u.email',
...
router.post(
  ...
  [
    ...
    [
      ...
      check('pay_id', 'El nombre de pago ingresado es incorrecto. Por favor intentalo nuevamente.')
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (user.length === 0) {
        options = {
          email,
          subject: `${req.user.name} te ha invitado a unirte a Pago INSIBS`, template: 'invitation',
          variables: JSON.stringify({ sender_name: req.user.name })
        }
      } else {
        options = {
          email,
          subject: 'Tienes una nueva solicitud de cobro', template: 'transfer_request',
          variables: JSON.stringify({
            name: user[0].first_name,
            sender_name: req.user.name, sender_cedula: req.user.cedula,
