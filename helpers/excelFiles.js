...
const createPendingReport = (fees, payments, currencyId) => {
  const workbook = new Excel.Workbook();
  ...
  if (+currencyId === 1) columns.push({ header: "BANCO", key: "bank_name", width: 20 });
  if (+currencyId === 2) columns.push({ header: "TIPO DE PAGO", key: "payment_type", width: 15 });
  ...
  if (fees.length > 0) {
    for (let [i, fee] of fees.entries()) {
      const values = {
        ...
        fee_amount: formatAmount(fee.debit.feeAmount) + " " + fee.debit.currency.symbol,
        supplier: fee.debit.supplier.name,
      };
      if (fee.debit.paymentType === "account") values.bank_name = fee.debit.bankName;
      if (fee.debit.paymentType === "zelle" || fee.debit.paymentType === "paypal") values.payment_type = fee.debit.paymentType;
  ...
  if (payments.length > 0) {
    for (let [i, payment] of payments.entries()) {
      const values = {
        ...
        fee_amount: formatAmount(payment.amount) + " " + payment.currency.symbol,
        supplier: payment.supplier.name,
      };
      if (payment.paymentType === "account") values.bank_name = payment.bankName;
      if (payment.paymentType === "zelle" || payment.paymentType === "paypal") values.payment_type = payment.paymentType;
      worksheet.addRow(values);
    }
  }
  ...
  totalRowValues[5] = `PENDIENTE:  ${formatAmount(totals.totalPrice)} ${totals.symbol}`;
...
function getTotalFromFees(fees, payments) {
  ...
  if (fees.length > 0) symbol = fees[0].debit.currency.symbol;
  if (payments.length > 0) symbol = payments[0].currency.symbol;

  for (fee of fees) totalPrice += +fee.debit.feeAmount;
  for (payment of payments) totalPrice += +payment.amount;

  return { totalReports, totalPrice, symbol };
}
