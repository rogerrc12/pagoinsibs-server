...
const generatePdfFooter = (doc) => {
  doc
    .fontSize(9)
    .text("Los pagos son reflejados en tu cuenta en un lapso no mayor a 24 horas.",
      50,
      760,
      { align: "center", width: 500 })
    .text(
      "Si tienes alguna duda contáctanos al 0500-4674270. Gracias por usar nuestros servicios",
      50,
      775,
      { align: "center", width: 500 }
    );
...
const generateDebitInfo = (doc, debit) => {
  ...
  doc
    .fillColor("#444444")
    .fontSize(18)
    .text(`Recibo de domiciliación #${debit.id}`, 50, 160);
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .text(formatAmountBs(debit.feeTotalAmount), 135, 245)
    .font("Helvetica")
    ...
    .font("Helvetica-Bold").text('No. de cuenta:', 350, 305).font("Helvetica").text(debit.accNumber, 425, 305)
    .font("Helvetica-Bold").text('Tipo de cuenta:', 350, 320).font("Helvetica").text(debit.accType, 430, 320)
    .moveDown();
...
const generateFeesTable = (doc, fees) => {
  ...
  doc
    .font("Helvetica-Bold")
    .text('Cuota no.', 50, 372)
    .text('Fecha de cobro', 200, 372)
    .text('Status', 330, 372)
    .text('Monto de cuota', 280, 372, { align: 'right' })
...
const generatePaymentInformation = (doc, payment) => {
  ...
  doc
    .fillColor("#444444")
    .fontSize(18)
    .text(`Recibo pago único #${payment.id}`, 50, 160);
...
const generateInvoicePaymentTable = (doc, payment) => {
  doc
    .font("Helvetica-Bold")
    .text('Cuenta a debitar', 50, 372)
    .text('Descripción', 200, 372)
    .text('Status', 330, 372)
    .text('Monto', 280, 372, { align: 'right' })
